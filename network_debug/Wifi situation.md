...
(Corrected 2026-04-12: there are 6 eeros, all model "eero Pro" per the eero API.
Connection type of each not re-verified.)
...
~~There are also two devices listed as Gateway eero Pro. I don't know where
these devices are. They could be inside the network panel. There shouldn't
be two gateways, but it seems to work, so I'm not messing with them.~~
(Corrected 2026-04-12: there is only **one** real gateway — Bedroom, at
`192.168.4.1`. The eero API reports `is_gateway: false` for every node, so
label alone is misleading; the real gateway is identified by its mgmt IP
matching the LAN gateway.) They are labeled Unknown Gateway and Living
Room, though I don't think it's in the living room. There is also a third
eero device labeled Unknown. It's usually been off-line but as I'm typing
might not be anymore. Though it was last active on 10/31/2025. (Still DOWN
as of 2026-04-12.)
